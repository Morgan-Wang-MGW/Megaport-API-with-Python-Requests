...
def write_service_details(vxc_service_details):
    try:
        with open('vxc_service_validate_details.json', 'w') as vxc_service_detials_file:
            json.dump(vxc_service_details, vxc_service_detials_file, indent=4)
        print("Successfully wrote to vxc_service_validate_details.json")
    except IOError as e:
...
payload = json.dumps([
  {
      "productName": "My_test_VXC",
      # A descriptive name for the VXC.
      ...
      "associatedVxcs": [
          {
              "productName": "Python API Test VXC 202503232203",
              "rateLimit": 500,
              "aEnd": {
                  "vlan": 2001
              },
...
validate_response = requests.post(f"{vxc_order_url}/validate",
...
if validate_response.status_code == 200:
    ...
    vxc_service_details = validate_response.json()
    print("VXC order validate service details : ", vxc_service_details)

    # Add a timestamp
    timestamp_now = datetime.datetime.now()
    vxc_service_details['VXC validate timestamp'] = timestamp_now.strftime("%Y-%m-%d  %H-%M-%S")

    # Write the validate details to vxc_service_validate_details.json
