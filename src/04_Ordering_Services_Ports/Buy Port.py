...
def write_service_details(port_service_details):
    try:
        with open('port_service_details.json', 'w') as port_service_detials_file:
...
if order_response.status_code == 200:
    ...
    print("Port order service details : ", port_service_details)

    # Add a timestamp
    timestamp_now = datetime.datetime.now()
    port_service_details['Port order timestamp'] = timestamp_now.strftime("%Y-%m-%d  %H-%M-%S")
