...
def write_service_details(port_service_details):
    try:
        with open('lag_validate_details.json', 'w') as lag_service_detials_file:
            json.dump(port_service_details, lag_service_detials_file, indent=4)
        print("Successfully wrote locations to lag_validate_details.json")
    except IOError as e:
...
if validate_response.status_code == 200:
    ...
    print("LAG order validate service details : ", port_service_details)

    # Add a timestamp
    timestamp_now = datetime.datetime.now()
    port_service_details['LAG validate timestamp'] = timestamp_now.strftime("%Y-%m-%d  %H-%M-%S")
